...
if grep -q '^#define CONFIG_ANDROID_BINDER_IPC 1' "${src_dir}/include/generated/autoconf.h"; then
	echo "Skipping legacy binder module; kernel ${KVER} provides in-tree binder support"
else
	(
	cd binder || exit 1
	make KERNEL_SRC="../${src_dir}" CC=${CC} HOSTCC=${CC}
