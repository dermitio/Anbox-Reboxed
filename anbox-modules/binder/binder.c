...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0)
#define binder_mmap_write_lock(mm) mmap_write_lock(mm)
#define binder_mmap_write_unlock(mm) mmap_write_unlock(mm)
#else
#define binder_mmap_write_lock(mm) down_write(&(mm)->mmap_sem)
#define binder_mmap_write_unlock(mm) up_write(&(mm)->mmap_sem)
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(7, 0, 0)
static int binder_security_set_context_mgr(struct binder_proc *proc)
{
	const struct cred *cred;
	int ret;

	cred = get_task_cred(proc->tsk);
	ret = security_binder_set_context_mgr(cred);
	put_cred(cred);

	return ret;
}

static int binder_security_transaction(struct binder_proc *from,
				       struct binder_proc *to)
{
	const struct cred *from_cred;
	const struct cred *to_cred;
	int ret;

	from_cred = get_task_cred(from->tsk);
	to_cred = get_task_cred(to->tsk);
	ret = security_binder_transaction(from_cred, to_cred);
	put_cred(to_cred);
	put_cred(from_cred);
...
static int binder_security_transfer_binder(struct binder_proc *from,
					   struct binder_proc *to)
{
	const struct cred *from_cred;
	const struct cred *to_cred;
	...
	from_cred = get_task_cred(from->tsk);
	to_cred = get_task_cred(to->tsk);
	ret = security_binder_transfer_binder(from_cred, to_cred);
	put_cred(to_cred);
	put_cred(from_cred);
...
static int binder_security_transfer_file(struct binder_proc *from,
					 struct binder_proc *to,
					 struct file *file)
{
	const struct cred *from_cred;
	...
	from_cred = get_task_cred(from->tsk);
	to_cred = get_task_cred(to->tsk);
	ret = security_binder_transfer_file(from_cred, to_cred, file);
	put_cred(to_cred);
	put_cred(from_cred);
...
static int binder_security_set_context_mgr(struct binder_proc *proc)
{
	return security_binder_set_context_mgr(proc->tsk);
}

static int binder_security_transaction(struct binder_proc *from,
				       struct binder_proc *to)
{
	return security_binder_transaction(from->tsk, to->tsk);
}

static int binder_security_transfer_binder(struct binder_proc *from,
					   struct binder_proc *to)
{
	return security_binder_transfer_binder(from->tsk, to->tsk);
}
...
static int binder_security_transfer_file(struct binder_proc *from,
					 ...
					 struct file *file)
{
	return security_binder_transfer_file(from->tsk, to->tsk, file);
}
#else
static int binder_security_set_context_mgr(struct binder_proc *proc)
{
				   ...
				   struct binder_thread *thread)
{
	...
	if (binder_security_transfer_binder(proc, target_proc))
		return -EPERM;
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0)
	ret = binder_security_transfer_file(proc, target_proc, file);
	if (ret < 0) {
		ret = -EPERM;
		goto err_security;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0)
		if (binder_security_transaction(proc, target_proc) < 0) {
			return_error = BR_FAILED_REPLY;
			goto err_invalid_target_handle;
		}
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0)
	ret = binder_security_set_context_mgr(proc);
	if (ret < 0)
		goto out;
#endif
...
static int binder_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	vm_flags_mod(vma, VM_DONTCOPY, VM_MAYWRITE);
#else
...
static int __init binder_init(void)
{
	...
	binder_class = class_create("binder");
#else
...
MODULE_DESCRIPTION("Legacy Android binder IPC driver");
