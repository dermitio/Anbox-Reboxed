...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(7, 0, 0)
static int (*vmap_pages_range_noflush_ptr)(unsigned long start, unsigned long end, pgprot_t prot, struct page **pages, unsigned int page_shift) = NULL;
static void (*vunmap_range_noflush_ptr)(unsigned long, unsigned long) = NULL;
#else
...
#endif
{
	...
	if (!zap_page_range_single_ptr)
		zap_page_range_single_ptr = kallsyms_lookup_name("zap_page_range_single");
	zap_page_range_single_ptr(vma, address, size, NULL);
#else
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(7, 0, 0)
	if (!vmap_pages_range_noflush_ptr)
		vmap_pages_range_noflush_ptr = kallsyms_lookup_name("vmap_pages_range_noflush");
	return vmap_pages_range_noflush_ptr(start, start + size, prot, pages, PAGE_SHIFT);
#else
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(7, 0, 0)
	if (!vunmap_range_noflush_ptr)
		vunmap_range_noflush_ptr = kallsyms_lookup_name("vunmap_range_noflush");
	vunmap_range_noflush_ptr(addr, addr + size);
#else
