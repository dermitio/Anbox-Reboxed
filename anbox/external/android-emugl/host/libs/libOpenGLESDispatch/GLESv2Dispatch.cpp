...
static GLESv2Dispatch *s_gles2_dispatch = NULL;
...
#define LOOKUP_EXT_SYMBOL(return_type,function_name,signature,callargs) \
    ...
    s_gles2_dispatch = dispatch_table;
...
void *gles2_dispatch_get_proc_func(const char *name, void *userData)
{
    ...
    // gles2_dispatch_init() resolves the complete table before render threads
    // are started. Reusing it avoids calling eglGetProcAddress repeatedly from
    // worker threads; recent Mesa releases can disturb that thread's EGL
    // client-API state while resolving the extension portion of this table.
#define LOOKUP_CACHED_PROC(return_type, function_name, signature, callargs) \
    if (!func && s_gles2_dispatch && \
        strcmp(name, #function_name) == 0) { \
        func = reinterpret_cast<void *>(s_gles2_dispatch->function_name); \
    }
    LIST_GLES2_FUNCTIONS(LOOKUP_CACHED_PROC, LOOKUP_CACHED_PROC)
#undef LOOKUP_CACHED_PROC
