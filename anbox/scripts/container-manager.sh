...
start() {
	...
	# Load kernel support. BinderFS is preferred on modern kernels; fall
	# back to the legacy binder module only when BinderFS is unavailable.
	...
	if grep -qw binder /proc/filesystems ; then
		...
		if grep -q "binder $SNAP_COMMON/binderfs" /proc/mounts ; then
