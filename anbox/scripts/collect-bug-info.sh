...
echo "need to have a better insight into the Anbox Reboxed system when something goes"
echo "wrong."
echo
...
echo
echo "Collecting Anbox Reboxed log files from legacy-compatible paths ... "
...
CURDIR=$PWD
(cd $TMPDIR; zip -r $CURDIR/anbox-reboxed-system-diagnostics-$(date --rfc-3339=date --utc).zip *)
rm -rf $TMPDIR
echo "DONE!"
