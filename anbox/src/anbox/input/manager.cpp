...
bool Manager::create_uinput_devices() {
  for (const auto &entry : devices_)
    if (!entry.second->create_uinput_device()) return false;
  return true;
}

std::vector<std::string> Manager::uinput_event_paths() const {
  std::vector<std::string> paths;
  for (const auto &entry : devices_)
    paths.push_back(entry.second->uinput_event_path());
  return paths;
}
