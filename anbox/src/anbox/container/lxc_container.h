...
class LxcContainer : public Container {
 ...
 private:
  ...
  std::vector<std::string> runtime_directories_to_restore_;
};

// get_id_map() is published for unit testing
std::vector<std::string> get_id_map(uid_t uid, gid_t gid);

// Graphics compatibility overlays must be selected as one coherent guest ABI.
// Published for focused unit testing of transport-specific mount selection.
bool should_mount_graphics_compatibility_bind(bool modern_gfxstream,
                                              const std::string &bind_mount);
}
#endif
