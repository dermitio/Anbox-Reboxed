...
std::recursive_mutex OpenGlesMessageProcessor::global_lock{};

std::recursive_mutex& OpenGlesMessageProcessor::host_gles_lock() {
  return global_lock;
}
...
OpenGlesMessageProcessor::OpenGlesMessageProcessor(
  ...
  INFO("OpenGL qemu pipe client flags bytes=[%02x %02x %02x %02x] value=%u",
       client_flags & 0xff, (client_flags >> 8) & 0xff,
       (client_flags >> 16) & 0xff, (client_flags >> 24) & 0xff,
       client_flags);
