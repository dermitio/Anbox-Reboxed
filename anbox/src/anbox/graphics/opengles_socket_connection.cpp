...
std::string bytes_to_hex(const std::uint8_t *data, std::size_t size) {
  ...
  const auto logged_size = std::min<std::size_t>(size, 64);
  for (std::size_t i = 0; i < logged_size; ++i) {
    if (i != 0)
      output << ' ';
...
void OpenGlesSocketConnection::on_read_size(const boost::system::error_code& error, std::size_t bytes_read) {
  if (error) {
    WARNING("OpenGL qemu pipe read stopped: %s", error.message().c_str());
    connections_->remove(id());
    return;
  }

  const auto data_hex = bytes_to_hex(buffer_.data(), bytes_read);
  TRACE("OpenGL qemu pipe received %zu bytes=[%s]", bytes_read,
        data_hex.c_str());
