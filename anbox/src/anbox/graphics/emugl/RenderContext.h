...
class RenderContext {
 public:
  ...
  // |guestGlesVersion| is the version requested by Android. Versions 2 and 3
  // currently share the GLES2-compatible host decoder. Preserve the request
  // for diagnostics, while capability queries truthfully report GLES2.
  static RenderContext* create(EGLDisplay display, EGLConfig config,
                               EGLContext sharedContext,
                               unsigned int guestGlesVersion = 1);
  ...
  // Return the GLES version Android requested for this context.
  unsigned int guestGlesVersion() const { return mGuestGlesVersion; }
 ...
 private:
  ...
  EGLContext mContext;
  bool mIsGl2;
  unsigned int mGuestGlesVersion;
