...
int RendererConfigList::chooseConfig(const EGLint* attribs, EGLint* configs,
  ...
  // The guest uses window surfaces and Android-only config constraints. The
  // host implementation backs guest windows with pbuffers and does not expose
  // EGL_ANDROID_recordable or EGL_ANDROID_framebuffer_target, so translate
  // those constraints before calling host EGL. Both Android attributes are
  // properties of the guest transport here, not host config capabilities.
  bool hasSurfaceType = false;
  std::vector<EGLint> hostAttribs;
  for (int n = 0; attribs[n] != EGL_NONE; n += 2) {
    const EGLint attribute = attribs[n];
    EGLint value = attribs[n + 1];
    if (attribute == EGL_RECORDABLE_ANDROID ||
        attribute == EGL_FRAMEBUFFER_TARGET_ANDROID) {
      continue;
    }
    ...
    hostAttribs.push_back(attribute);
  ...
  if (!s_egl.eglChooseConfig(mDisplay, hostAttribs.data(),
                             matchedConfigs, numHostConfigs, &numHostConfigs)) {
    const EGLint error = s_egl.eglGetError();
    WARNING("Guest EGL config query rejected by host: error=%#x", error);
    numHostConfigs = 0;
  }
