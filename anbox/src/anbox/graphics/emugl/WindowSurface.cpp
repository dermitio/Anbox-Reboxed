...
bool WindowSurface::resize(unsigned int p_width, unsigned int p_height) {
  ...
  if (mSurface == EGL_NO_SURFACE) {
    const EGLint egl_error = s_egl.eglGetError();
    EGLint config_id = 0;
    EGLint surface_type = 0;
    EGLint max_width = 0;
    EGLint max_height = 0;
    s_egl.eglGetConfigAttrib(mDisplay, mConfig, EGL_CONFIG_ID, &config_id);
    s_egl.eglGetConfigAttrib(mDisplay, mConfig, EGL_SURFACE_TYPE, &surface_type);
    s_egl.eglGetConfigAttrib(mDisplay, mConfig, EGL_MAX_PBUFFER_WIDTH, &max_width);
    s_egl.eglGetConfigAttrib(mDisplay, mConfig, EGL_MAX_PBUFFER_HEIGHT, &max_height);
    fprintf(stderr,
            "WINDOWSURFACE_FAIL size=%ux%u egl_error=%#x config_id=%d "
            "surface_type=%#x max=%dx%d\n",
            p_width, p_height, egl_error, config_id, surface_type,
            max_width, max_height);
    fflush(stderr);
    ERROR("Failed to create/resize pbuffer (EGL error %#x)", egl_error);
    return false;
  }
