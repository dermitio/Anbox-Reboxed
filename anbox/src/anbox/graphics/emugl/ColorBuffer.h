...
class ColorBuffer {
 public:
  ...
  // Copy the currently visible texture into this buffer. Both buffers remain
  // independent after this call; the caller must serialize host GLES access.
  bool snapshotFrom(ColorBuffer& source);

  // Diagnostic-only readback for a caller that already owns a compatible
  // current host context. Unlike readPixels(), this never switches EGL
  // contexts, making it safe in the presentation path.
  bool sampleCurrentTexture(GLubyte rgba[4]);
