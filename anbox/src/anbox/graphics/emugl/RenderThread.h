...
class RenderThread : public emugl::Thread {
 public:
  ...
  static RenderThread* create(const std::shared_ptr<Renderer>& renderer, IOStream* stream, std::recursive_mutex &m);
 ...
 private:
  ...
  RenderThread(const std::shared_ptr<Renderer>& renderer, IOStream* stream, std::recursive_mutex &m);
