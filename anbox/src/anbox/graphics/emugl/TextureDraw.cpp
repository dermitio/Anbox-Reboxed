...
GLuint createShader(GLint shaderType, const char* shaderText) {
  ...
  if (success == GL_FALSE) {
    GLchar messages[512] = {};
    s_gles2.glGetShaderInfoLog(shader, sizeof(messages), nullptr, messages);
    ERROR("Could not compile TextureDraw shader type=%#x: %s", shaderType,
          messages);
...
const char kFragmentShaderSource[] =
    ...
    "uniform lowp float invertTextureY;\n"

    "void main(void) {\n"
    "  lowp vec2 coord = vec2(outCoord.x, mix(outCoord.y, 1.0 - outCoord.y, invertTextureY));\n"
...
TextureDraw::TextureDraw(EGLDisplay)
  ...
  mInvertTextureYSlot =
      s_gles2.glGetUniformLocation(mProgram, "invertTextureY");
...
bool TextureDraw::draw(GLuint texture, bool invert_texture_y) {
  if (!mProgram) {
    ERROR(" No program");
    return false;
  }
  ...
  s_gles2.glUniform1f(mInvertTextureYSlot, invert_texture_y ? 1.0f : 0.0f);
