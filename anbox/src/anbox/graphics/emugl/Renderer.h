...
class Renderer : public anbox::graphics::Renderer {
 public:
  ...
  EGLint nativeVisualId() const { return m_nativeVisualId; }
  ...
  // SDL lifecycle notifications invalidate input mapping immediately. A new
  // valid snapshot is published only by draw(), after querying EGL and
  // applying the matching glViewport.
  void updateHostWindow(EGLNativeWindowType native_window, int window_width,
                        int window_height, const char* reason);
  bool renderedViewport(EGLNativeWindowType native_window,
  ...
  // |p_guestGlesVersion| is the GLES version requested by Android. GLES 2 and
  // 3 currently use the same compatible host decoder.
  // Return a new handle value, which will be 0 in case of error.
  HandleType createRenderContext(int p_config, HandleType p_share,
                                 unsigned int p_guestGlesVersion = 1);
  ...
  // Preserve resources briefly when a GL transport pipe closes. Android's
  // modern graphics stack can create a drawable on one pipe and bind it from
  // a long-lived context after that creator pipe has reached EOF.
  void deferCurrentThreadResources();
  ...
  bool retainPostedColorBuffer(HandleType p_colorbuffer, int* width,
                               int* height);
  ...
  bool retain_frame(RenderableList& renderables) override;
  void release_frame(const RenderableList& renderables) override;
  void set_redraw_requester(std::function<void()> requester) override;
  // Modern gfxstream owns Android's ColorBuffers. Its post callback publishes
  // one immutable CPU snapshot here; presentation retains only the newest.
  void enableExternalFrameSource(int width, int height);
  void disableExternalFrameSource();
  void postExternalFrame(int width, int height, int y_direction, int format,
                         int type, const unsigned char* pixels);
 ...
 private:
  ...
  bool recreateWindowSurfaceLocked(RendererWindow* window);
  void invalidateViewportLocked(RendererWindow* window, const char* reason);
  bool releaseWindowSurfaceColorBufferLocked(HandleType windowHandle,
                                             HandleType colorBufferHandle,
                                             const char* source);
  void reapDeferredResourcesLocked();
  void cancelDeferredContextLocked(HandleType context);
  void cancelDeferredWindowLocked(HandleType window);
  void destroyDeferredWindowLocked(HandleType window, const char* source);
 ...
 private:
  ...
  typedef std::chrono::steady_clock DeferredClock;
  typedef std::map<HandleType, DeferredClock::time_point> DeferredResourceMap;
  DeferredResourceMap m_deferredContexts;
  DeferredResourceMap m_deferredWindows;
  ...
  std::map<EGLNativeWindowType, std::pair<int, int>> m_pendingWindowSizes;
  std::function<void()> m_redrawRequester;
  bool m_externalFrameSource = false;
  int m_externalExpectedWidth = 0;
  int m_externalExpectedHeight = 0;
  int m_externalFrameWidth = 0;
  int m_externalFrameHeight = 0;
  int m_externalFrameYDirection = 1;
  int m_externalFrameFormat = GL_RGBA;
  int m_externalFrameType = GL_UNSIGNED_BYTE;
  std::uint64_t m_externalFrameGeneration = 0;
  std::uint64_t m_externalUploadedGeneration = 0;
  std::vector<unsigned char> m_externalFramePixels;
  GLuint m_externalFrameTexture = 0;
