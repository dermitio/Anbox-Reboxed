...
int cli::CommandWithSubcommands::run(const cli::Command::Context& ctxt) {
  ...
  pdesc.add("command", 1).add("arguments", -1);
  ...
  try {
    ...
    // Keep the original ordering of subcommand arguments. Collecting
    // unrecognised options before positional values turns e.g.
    // "install --source file --container default" into
    // "--source --container file default", which breaks external tools.
    const auto command_name = vm["command"].as<std::string>();
    const auto command_position = std::find(ctxt.args.begin(), ctxt.args.end(),
                                            command_name);
    std::vector<std::string> child_arguments;
    if (command_position != ctxt.args.end())
      child_arguments.assign(std::next(command_position), ctxt.args.end());

    // Commands without their own option parser (for example the Android
    // convenience tools) must not receive --help as an argument to execute.
    // Handle it uniformly at each subcommand boundary so nested commands and
    // simple leaf commands both provide help without requiring their backing
    // service or container to be running.
    const bool child_has_subcommands =
        std::dynamic_pointer_cast<CommandWithSubcommands>(cmd) != nullptr;
    if (!child_has_subcommands &&
        (std::find(child_arguments.begin(), child_arguments.end(), "--help") !=
             child_arguments.end() ||
         std::find(child_arguments.begin(), child_arguments.end(), "-h") !=
             child_arguments.end())) {
      cmd->help(ctxt.cout);
      return EXIT_SUCCESS;
    }
    ...
    return cmd->run(
        cli::Command::Context{ctxt.cin, ctxt.cout, child_arguments});
