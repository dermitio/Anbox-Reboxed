...
namespace {
void serialize_to_array_or_throw(const google::protobuf::MessageLite &message,
                                 std::uint8_t *data, size_t size) {
  if (size > static_cast<size_t>(std::numeric_limits<int>::max()) ||
      !message.SerializeToArray(data, static_cast<int>(size)))
...
void Channel::send_event(google::protobuf::MessageLite const &event) {
  VariableLengthArray<2048> buffer{static_cast<size_t>(event.ByteSizeLong())};
  serialize_to_array_or_throw(event, buffer.data(), buffer.size());
...
protobuf::rpc::Invocation Channel::invocation_for(
  ...
  serialize_to_array_or_throw(*request, buffer.data(), buffer.size());
...
void Channel::send_message(const std::uint8_t &type,
  ...
  std::copy(header_bytes, header_bytes + sizeof(header_bytes),
            send_buffer.begin());
  serialize_to_array_or_throw(message, send_buffer.data() + sizeof(header_bytes), size);
