  ...
  if (!x) \
    ...
    if (missing_features.size() > 0 && !is_whitelisted && !sanity_check_for_features()) {
      std::cerr << "The CPU of your computer (" << brand_string << ") does not support all" << std::endl
                << "features Anbox Reboxed requires." << std::endl
    ...
    std::cout << "Your computer does meet all requirements to run Anbox Reboxed" << std::endl;

    return EXIT_SUCCESS;
#else
    std::cerr << "You're running Anbox Reboxed on a not yet supported architecture" << std::endl;
    return EXIT_FAILURE;
#endif
  });
}
