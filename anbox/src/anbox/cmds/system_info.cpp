...
class SystemInformation {
 public:
  ...
  std::string to_text() const {
    ...
    s << "product: Anbox Reboxed" << std::endl
      << "version: "
      << anbox::build::version
      << std::endl;
    ...
    s << "graphics:" << std::endl
      << "  acceleration: " << graphics_info_.acceleration << std::endl
      << "  renderer: " << graphics_info_.gles2_renderer << std::endl
      << "  compatibility-decoder:" << std::endl
      << "    max-client-api: OpenGL ES 2.0" << std::endl
      << "    guest-gles3: unsupported" << std::endl
      << "  egl:" << std::endl
      ...
      << "    version: " << graphics_info_.gles2_version << std::endl
 ...
 private:
  ...
  void collect_kernel_info() {
    ...
    kernel_info_.binderfs = fs::exists(binderfs_control_path);
    ...
    if (display != EGL_NO_DISPLAY) {
      ...
      if (s_egl.eglChooseConfig(display, config_attribs, &config, 1, &n) && n > 0) {
        ...
        if (context != EGL_NO_CONTEXT) {
          ...
          graphics_info_.gles2_renderer = gl_safe_get_string(GL_RENDERER);
          ...
          auto identity = graphics_info_.gles2_vendor + " " +
                          graphics_info_.gles2_renderer;
          std::transform(identity.begin(), identity.end(), identity.begin(),
                         [](unsigned char value) {
                           return static_cast<char>(std::tolower(value));
                         });
          graphics_info_.acceleration = "hardware";
          for (const auto* marker : {"llvmpipe", "softpipe", "swrast",
                                     "lavapipe", "software rasterizer",
                                     "swiftshader"}) {
            if (identity.find(marker) != std::string::npos) {
              graphics_info_.acceleration = "software";
              break;
            }
          }
  ...
  struct {
    ...
    std::string acceleration = "unknown";
    std::string gles2_vendor = "n/a";
    std::string gles2_renderer = "n/a";
