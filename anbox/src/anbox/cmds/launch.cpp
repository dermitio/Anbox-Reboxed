...
anbox::cmds::Launch::Launch()
  ...
  action([this](const cli::Command::Context& context) {
    if (context.args.size() > 2) {
      ERROR("Usage: anbox-reboxed launch <package> [component]");
      return EXIT_FAILURE;
    }

    if (!context.args.empty()) {
      intent_.package = context.args[0];
      if (context.args.size() == 2)
        intent_.component = context.args[1];
    }

    if (!intent_.valid()) {
      ERROR("The intent you provided is invalid. Please provide a correct launch intent.");
      ERROR("For example: anbox-reboxed launch com.example.app .MainActivity");
      return EXIT_FAILURE;
    }
    ...
    // The Android 15 image no longer contains org.anbox.appmgr, anboxd, or
    // org.anbox.PlatformService.  Do not wait on the legacy bridge's Ready()
    // flag; launch through the modern ActivityTaskManager command interface.
    unsigned int n = 0;
    ...
    std::vector<std::string> command{"/system/bin/cmd", "activity",
                                     "start-activity", "-W"};
    if (!intent_.component.empty()) {
      auto component = intent_.component;
      if (component.find('/') == std::string::npos)
        component = intent_.package + "/" + component;
      command.insert(command.end(), {"-n", component});
    } else {
      command.insert(command.end(), {"-a", intent_.action.empty()
                                               ? "android.intent.action.MAIN"
                                               : intent_.action,
                                     "-c", "android.intent.category.LAUNCHER",
                                     "-p", intent_.package});
    }
    if (!intent_.type.empty())
      command.insert(command.end(), {"-t", intent_.type});
    if (!intent_.uri.empty())
      command.insert(command.end(), {"-d", intent_.uri});
