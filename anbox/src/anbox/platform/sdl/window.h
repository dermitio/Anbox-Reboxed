...
class Window : public std::enable_shared_from_this<Window>, public wm::Window {
 public:
  ...
  class Observer {
   public:
    ...
    virtual void window_lost_focus(const Id &id) = 0;
    virtual void window_hidden(const Id &id) = 0;
    ...
    virtual void window_resized(const Id &id, const std::int32_t &x,
                                const std::int32_t &y) = 0;
    virtual void window_lifecycle_changed(const Id &id,
                                          const char *reason) = 0;
  };
  ...
  bool process_window_control_event(const SDL_Event &event);
 ...
 private:
  enum class WindowControl { none, close, maximize };

  static SDL_HitTestResult on_window_hit(SDL_Window *window, const SDL_Point *pt, void *data);

  WindowControl window_control_at(int x, int y) const;
  void close();
  ...
  WindowControl pressed_window_control_{WindowControl::none};
};
}
#endif
