...
void SocketConnection::on_read_size(const boost::system::error_code& error, std::size_t bytes_read) {
  if (error) {
    ...
    DEBUG("Qemu pipe lifecycle: close connection=%d service=%s age_ms=%lld error=%s",
          id_, name_.empty() ? "unidentified" : name_.c_str(),
          static_cast<long long>(age), error.message().c_str());
  ...
  if (processor_->process_data(data))
    ...
    const auto age = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - created_at_).count();
    WARNING("Qemu pipe lifecycle: processor rejected connection=%d service=%s age_ms=%lld bytes=%zu",
            id_, name_.empty() ? "unidentified" : name_.c_str(),
            static_cast<long long>(age), bytes_read);
    connections_->remove(id());
  }
}
}
