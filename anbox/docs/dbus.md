...
Anbox Reboxed retains the compatibility-sensitive `org.anbox` D-Bus interfaces for launching applications (`anbox-reboxed launch`) and setting sensor values. You can query them with:
```
...
```
anbox-reboxed launch --package=org.anbox.appmgr --component=org.anbox.appmgr.AppViewActivity
```
